...
public:
    ...
    /**
     * 设置取消标志，下次TTS结束后进入待命而不是聆听
     */
    void SetCancelAfterSpeaking(bool cancel) { cancel_after_speaking_ = cancel; }
...
private:
    ...
    bool cancel_after_speaking_ = false;  // 用户取消后，TTS结束进入待命而不是聆听
    int clock_ticks_ = 0;
    int silence_ticks_ = 0;  // 静音计时（秒）
